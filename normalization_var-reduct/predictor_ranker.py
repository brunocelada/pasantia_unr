...
def write_dict_file(model_dict, folder_path):
    """
    Crea la carpeta si no existe y escribe el diccionario en 'organized_dict.txt'.
    """
    os.makedirs(folder_path, exist_ok=True)
    txt_path = os.path.join(folder_path, 'organized_dict.txt')
...
def create_sort_sheet(writer, model_dict, descriptors, top_value):
    ...
    for col in descriptors:
        ...
        rev = col.startswith("R2")
        sorted_models = sorted(valid_items, key=lambda x: x[1], reverse=rev)
        models_list = [m for m, _ in sorted_models[:top_value]]

        # Rellenar si falta
        if len(models_list) < top_value:
            models_list += [None] * (top_value - len(models_list))

        sort_data[col] = models_list
...
def create_ranking_sheet(writer, df_sort):
    ...
    for col in df_sort.columns:
        ...
        for model in df_sort[col]:
            ...
            if len(parts) == 3:
                ...
                red_cnt[parts[1]] += 1
                alg_cnt[parts[2]] += 1
    def counter_to_df(cnt, name='Name'):
    ...
    df_norm = counter_to_df(norm_cnt, name="Normalization")
    df_red = counter_to_df(red_cnt, name="Reduction")
    df_alg = counter_to_df(alg_cnt, name="Model")
    ...
    # === Nueva funcionalidad: combinaciones por pares ===
    df_norm_red, df_red_mod, df_norm_mod = create_pairwise_combinations_counts(df_rank)
    logger.info("Hoja 'Ranking': tablas de combinaciones por pares creadas.")

    df_norm_red.to_excel(writer, sheet_name='Ranking', index=False, startcol=16)
    df_red_mod.to_excel(writer, sheet_name='Ranking', index=False, startcol=20)
    df_norm_mod.to_excel(writer, sheet_name='Ranking', index=False, startcol=24)

# ----------------------------------------
# Función: create_pairwise_combinations_counts
# Descripción: Esta función recibe el df_rank, descompone los nombres "norm_red_model", 
# y genera 3 tablas de ranking para:
    # Normalización-Reducción
    # Reducción-Modelo
    # Normalización-Modelo
# ----------------------------------------
def create_pairwise_combinations_counts(df_rank):
    """
    Crea DataFrames de conteo para combinaciones:
    1) Normalización + Reducción
    2) Reducción + Modelo
    3) Normalización + Modelo

    Retorna 3 DataFrames con columnas: #Rank, Combination, N°Success
    """
    from collections import Counter

    # Extraer modelos del ranking
    models = df_rank['Model'].dropna().tolist()

    comb_norm_red = Counter()
    comb_red_mod = Counter()
    comb_norm_mod = Counter()

    for m in models:
        parts = m.split('_')
        if len(parts) == 3:
            norm, red, mod = parts
            comb_norm_red[f"{norm}_{red}"] += 1
            comb_red_mod[f"{red}_{mod}"] += 1
            comb_norm_mod[f"{norm}_{mod}"] += 1

    def counter_to_df(cnt, name='Combination'):
        items = cnt.most_common()
        rows = [(i+1, comb, val) for i, (comb, val) in enumerate(items)]
        return pd.DataFrame(rows, columns=['#Rank', name, 'N°Success'])

    df_norm_red = counter_to_df(comb_norm_red, name="Norm-Red")
    df_red_mod = counter_to_df(comb_red_mod, name="Red-Model")
    df_norm_mod = counter_to_df(comb_norm_mod, name="Norm-Model")

    return df_norm_red, df_red_mod, df_norm_mod
...
# ----------------------------------------
# Función: recalculate_excel
# Descripción: Recalcula todas las fórmulas del archivo Excel y lo guarda.
# ----------------------------------------
def recalculate_excel(excel_path):
    ...
    wb.close()
    app.quit()
    print(f"Archivo recalculado y guardado: {excel_path}")
...
def main():
    ...
    recalculate_excel(excel_path)

    # 2. Establece el directorio base y la carpeta de salida 2_Organizer_Folder.
    base_dir = os.path.dirname(os.path.abspath(excel_path)) or '.'
    organizer_folder = os.path.join(base_dir, '2_Organizer_Folder')

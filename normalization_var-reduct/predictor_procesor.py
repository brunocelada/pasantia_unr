...
import time
import winsound
import json  # Para guardar y cargar diccionarios como JSON
...
def generate_templates_and_calculate_outliers(template_path, data_dict, target_column_letter, output_folder,
                                              l_cell, w_cell, k_cell, v_cell):
    target_col_idx = column_index_from_string(target_column_letter)
    os.makedirs(output_folder, exist_ok=True)

    # Limpiar archivos existentes antes de iniciar
    open(os.path.join(os.path.dirname(output_folder), "outlier_dict_ddg.txt"), 'w').close()
    open(os.path.join(os.path.dirname(output_folder), "outlier_dict_ee.txt"), 'w').close()

    outlier_dict_ddg = {}
    outlier_dict_ee = {}

    for original_name, (values, subfolder) in data_dict.items():
        model_name = subfolder
        clean_name = f"Template_{model_name}"
        output_path = os.path.join(output_folder, f"{clean_name}.xlsx")

        # Copiar template y pegar columna
        ...
        # Guardar valores de ΔΔG y %ee en los archivos txt (modo append, línea por línea JSON)
        ...
        wb.save(output_path)
        logging.info(f"Template de {model_name} generado y hoja Outliers añadida.")
...
def main():
    ...
    output_folder = os.path.join(base_folder, "1-Templates-Generados")
    os.makedirs(output_folder, exist_ok=True)
    ...
    generate_templates_and_calculate_outliers(
        template_path, column_data, target_column, output_folder, l_cell, w_cell, k_cell, v_cell
    )

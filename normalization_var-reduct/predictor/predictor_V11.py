...
def apply_reduction(X_train, y_train, X_val, reduction_type, original_feature_names):
    ...
    if reduction_type == "pca":
        print("Aplicando PCA con selección de n_components por optimización de predicción...")
        ...
        if n_samples_total == 0 or n_features == 0:
            ...
            if not pca_n_components_options:
                ...
                try:
                    ...
                    print(f"Advertencia: GridSearchCV de PCA falló ({e}). Usando características originales.")
                    new_feature_names = original_feature_names 
                    reduction_info["status"] = f"Error en GridSearchCV: {e}"

    elif reduction_type == "pca_2":
        print("Aplicando PCA con selección de componentes por 95% de varianza explicada (Estrategia 2)...")
        try:
            reducer_object = PCA(n_components=0.95, random_state=RANDOM_STATE) 
            reducer_object.fit(X_train.values)
            X_train_transformed = reducer_object.transform(X_train.values)
            X_val_transformed = reducer_object.transform(X_val.values)
            new_feature_names = [f"PC{i+1}" for i in range(reducer_object.n_components_)]
            X_train_reduced_df = pd.DataFrame(X_train_transformed, columns=new_feature_names, index=X_train.index)
            X_val_reduced_df = pd.DataFrame(X_val_transformed, columns=new_feature_names, index=X_val.index)
            reduction_info["n_components_for_95_var"] = reducer_object.n_components_
            reduction_info["explained_variance_ratio"] = reducer_object.explained_variance_ratio_.tolist()
            reduction_info["total_explained_variance"] = sum(reducer_object.explained_variance_ratio_)
        except Exception as e_fallback:
            print(f"PCA (estrategia 2) falló: {e_fallback}. Usando características originales.")
            new_feature_names = original_feature_names 
            reduction_info["status"] = "PCA_2 falló, usando características originales"
        ...
        print("Aplicando PLS con selección de n_components por optimización de predicción...")
        max_comps = min(X_train.shape[0] -1, X_train.shape[1])
        if max_comps > 0:
            ...
            try:
                ...
                print(f"Advertencia: GridSearchCV de PLS falló ({e}). Usando fallback.")
                n_c = min(X_train.shape[0] -1, X_train.shape[1])
                if n_c > 0:
                    try:
                        ...
                        print(f"Fallback de PLS también falló: {e_fallback}. Usando características originales.")
                        new_feature_names = original_feature_names
                        reduction_info["status"] = "PLS falló, usando características originales"
                else: 
                    print("Advertencia: No hay suficientes características/muestras para el fallback de PLS.")
                    new_feature_names = original_feature_names
        else: 
            print("Advertencia: No hay suficientes características/muestras para PLS.")
            new_feature_names = original_feature_names
        ...
        print("Aplicando RFE con estimador lineal (Estrategia 1)...")
        if X_train.shape[1] > 0:
    ...
    elif reduction_type == "rfe_2": 
        print("Aplicando RFE con estimador RandomForest (Estrategia 2)...")
        if X_train.shape[1] > 0:
            # Usar un RandomForestRegressor más potente como estimador
            estimator_for_rfe = RandomForestRegressor(n_estimators=50, random_state=RANDOM_STATE, n_jobs=-1)
            cv_rfe = cv_folds_for_gridsearch
            if X_train.shape[0] < 2: print("Advertencia: No hay suficientes muestras para RFE CV."); reduction_info["status"] = "Omitido (muy pocas muestras para CV)"
            else:
                try:
                    ...
                    selected_mask = reducer_object.get_support()
                    new_feature_names = [name for i, name in enumerate(original_feature_names) if selected_mask[i]]
                    if not new_feature_names and X_train.shape[1] > 0: print("Advertencia: RFE_2 seleccionó 0 características.")
                    X_train_reduced_df = pd.DataFrame(X_train_transformed_np, columns=new_feature_names, index=X_train.index)
                    X_val_reduced_df = pd.DataFrame(X_val_transformed_np, columns=new_feature_names, index=X_val.index)
                    reduction_info["selected_features_names"] = new_feature_names
                except Exception as e: print(f"Error durante RFE_2: {e}. Usando características originales."); new_feature_names, reduction_info["status"] = original_feature_names, f"Error: {e}"
        else: print("Advertencia: No hay características para aplicar RFE_2."); reduction_info["status"] = "Omitido (0 características de entrada)"

    elif reduction_type == "sparsepca": 
        print("Aplicando SparsePCA con selección de n_components y alpha por optimización de predicción...")
        ...
        if n_samples_total == 0 or n_features == 0: print("Advertencia: SparsePCA no se puede aplicar con 0 muestras o 0 características. Omitiendo."); reduction_info["status"] = "Omitido (0 muestras/características)"
            ...
            if not spca_n_components_options: print(f"Advertencia: SparsePCA no puede determinar un rango válido de n_components (max_n_comp={max_n_comp}). Usando fallback."); reduction_info["status"] = "Omitido (rango de n_components inválido para GridSearchCV)"
                ...
                try:
                    ...
                    loadings_df = pd.DataFrame(reducer_object.components_.T, index=original_feature_names, columns=new_feature_names)
                    reduction_info['sparsepca_loadings'] = loadings_df.to_dict()
                except Exception as e: 
                    print(f"Advertencia: GridSearchCV de SparsePCA falló ({e}). Usando características originales.")
                    new_feature_names, reduction_info["status"] = original_feature_names, f"Error en GridSearchCV: {e}"

    elif reduction_type == "sparsepca_2":
        print("Aplicando SparsePCA con n_components por 95% de varianza y alpha=1.0 (Estrategia 2)...")
        try:
            # 1. Usar PCA para determinar n_components para 95% de varianza
            pca_for_n = PCA(n_components=0.95, random_state=RANDOM_STATE)
            pca_for_n.fit(X_train.values)
            n_comps = pca_for_n.n_components_
            
            # 2. Usar ese n_components en SparsePCA con alpha fijo
            alpha_val = 1.0
            reducer_object = SparsePCA(n_components=n_comps, alpha=alpha_val, random_state=RANDOM_STATE, max_iter=100000)
            reducer_object.fit(X_train.values)
            ...
            reduction_info["n_components_from_95_var_pca"] = n_comps
            reduction_info["alpha_fixed"] = alpha_val
            loadings_df = pd.DataFrame(reducer_object.components_.T, index=original_feature_names, columns=new_feature_names)
            reduction_info['sparsepca_loadings'] = loadings_df.to_dict()

        except Exception as e_fallback:
            print(f"SparsePCA (estrategia 2) falló: {e_fallback}. Usando características originales.")
            new_feature_names = original_feature_names 
            reduction_info["status"] = "SparsePCA_2 falló, usando características originales"
        ...
        if X_train.shape[1] > 0:
            print("Aplicando PFI con selección por criterio estadístico (Estrategia 1)...")
    ...
    elif reduction_type == "pfi_2":
        if X_train.shape[1] > 0:
            print("Aplicando PFI con selección por umbral de ruido (Estrategia 2)...")
            
            # 1. Crear copia y añadir característica de ruido
            X_train_with_noise = X_train.copy()
            noise_feature_name = "random_noise_feature"
            np.random.seed(RANDOM_STATE)
            X_train_with_noise[noise_feature_name] = np.random.randn(len(X_train))
            
            # 2. Entrenar modelo y calcular importancias en el dataset con ruido
            estimator_for_pfi = RandomForestRegressor(n_estimators=50, random_state=RANDOM_STATE, n_jobs=-1)
            estimator_for_pfi.fit(X_train_with_noise.values, y_train_ravel)
            
            result = permutation_importance(
                estimator_for_pfi, X_train_with_noise.values, y_train_ravel,
                n_repeats=10, random_state=RANDOM_STATE, n_jobs=-1
            )
            
            # 3. Encontrar la importancia del ruido
            noise_feature_index = X_train_with_noise.columns.get_loc(noise_feature_name)
            noise_importance_threshold = result.importances_mean[noise_feature_index]
            
            # 4. Seleccionar características reales que superen el umbral de ruido
            importances_real_features = result.importances_mean[:-1] # Todas excepto la última (ruido)
            selected_mask = importances_real_features > noise_importance_threshold
            
            new_feature_names = [name for i, name in enumerate(original_feature_names) if selected_mask[i]]
            
            if not new_feature_names and X_train.shape[1] > 0:
                 print("Advertencia: PFI_2 no seleccionó características. Usando todas las características originales como fallback.")
                 new_feature_names = original_feature_names
            ...
            reduction_info["n_selected_features"] = len(new_feature_names)
            reduction_info["selected_features_names"] = new_feature_names
            reduction_info["noise_importance_threshold"] = noise_importance_threshold
            reduction_info["pfi_importances"] = {name: imp for name, imp in zip(original_feature_names, importances_real_features)}

        else:
            print("Advertencia: No hay características para aplicar PFI_2.")
            reduction_info["status"] = "Omitido (0 características de entrada)"
...
# --- Funciones de graficación para PCA/PLSR/SparsePCA ---

def plot_pca_loadings(reducer_obj, original_feature_names, output_path):
    """
    Grafica los loadings de PCA para visualizar la importancia de las variables.
    """
    if not hasattr(reducer_obj, 'components_'):
        print("Advertencia: El objeto reductor no tiene el atributo 'components_'. No se puede generar el gráfico de loadings de PCA.")
        return

    loadings = reducer_obj.components_
    n_components = reducer_obj.n_components_
    component_labels = [f"PC{i+1}" for i in range(n_components)]

    loadings_df = pd.DataFrame(loadings.T, columns=component_labels, index=original_feature_names)

    plt.style.use('default')
    fig_width = max(10, n_components * 1.2)
    fig_height = max(8, len(original_feature_names) * 0.5)
    plt.figure(figsize=(fig_width, fig_height))
    
    sns.heatmap(loadings_df, annot=True, cmap='coolwarm', center=0, fmt='.2f', linewidths=.5)
    
    plt.title('Importancia de las Variables en los Componentes Principales (PCA Loadings)', pad=20, weight='bold')
...
def plot_plsr_loadings(reducer_obj, original_feature_names, output_path):
    """
    Grafica los loadings de PLSR para visualizar la importancia de las variables.
    """
    if not hasattr(reducer_obj, 'x_loadings_'):
        print("Advertencia: El objeto reductor no tiene el atributo 'x_loadings_'. No se puede generar el gráfico de loadings de PLSR.")
        return

    loadings = reducer_obj.x_loadings_
    n_components = reducer_obj.n_components
    component_labels = [f"PLSC{i+1}" for i in range(n_components)]

    loadings_df = pd.DataFrame(loadings, columns=component_labels, index=original_feature_names)
    ...
    plt.title('Importancia de las Variables en los Componentes PLS (PLSR Loadings)', pad=20, weight='bold')
...
def plot_sparsepca_loadings(reducer_obj, original_feature_names, output_path):
    """
    Grafica los loadings de SparsePCA para visualizar la importancia de las variables.
    """
    if not hasattr(reducer_obj, 'components_'):
        print("Advertencia: El objeto reductor no tiene el atributo 'components_'. No se puede generar el gráfico de loadings de SparsePCA.")
        return

    loadings = reducer_obj.components_
    n_components = reducer_obj.n_components_
    component_labels = [f"SPC{i+1}" for i in range(n_components)]
    ...
    plt.title('Importancia de las Variables en los Componentes Dispersos (SparsePCA Loadings)', pad=20, weight='bold')
    plt.xlabel('Componentes Dispersos')
    plt.ylabel('Variables Originales')
    plt.tight_layout()
    plt.savefig(output_path)
    plt.close()
...
def display_options_menu(scalers, reductions, models):
    """Muestra un menú con las opciones disponibles del script."""
    
    header = "| OPCIONES DISPONIBLES PARA EL SCRIPT |"
    line = "-" * len(header)
    ...
    col1_title = "ESCALADORES"
    col2_title = "REDUCCIONES/SELECCIÓN"
    col3_title = "MODELOS"
    
    w1, w2, w3 = 20, 25, 22
    
    header_cols = f"| {col1_title.ljust(w1)} | {col2_title.ljust(w2)} | {col3_title.ljust(w3)} |"
    line_cols = "-" * len(header_cols)
    
    print(header_cols)
    print(line_cols)
    
    for s, r, m in zip_longest(scalers, reductions, models, fillvalue=""):
        s_str = s.ljust(w1)
        r_str = r.ljust(w2)
        m_str = m.ljust(w3)
        print(f"| {s_str} | {r_str} | {m_str} |")
        
    print(line_cols)
    
    print("\nOtras Opciones:")
    print("  --trials:   Número de trials de Optuna o 'auto'. (Ej: --trials 100)")
    print("  --shap:     Activa los gráficos SHAP. (Ej: --shap on)")
    print("  --no_intro: Salta la introducción con el logo.")
    
    print("\nEjemplo de uso:")
    print("  python predictor_V11.py --training datos_entrenamiento.xlsx --validation datos_validacion.xlsx --scaler zscore --reduction pca_2 --model rf --trials 150\n")

def main():
    
    # --- Definición de Opciones para el Menú y Argparse ---
    scaler_choices = ["none", "zscore", "minmax", "decimal", "robust", "unit"]
    reduction_choices = ["none", "pca", "pca_2", "efa", "plsr", "lasso", "rfe", "rfe_2", "sparsepca", "sparsepca_2", "mihr", "pfi", "pfi_2"]
    model_choices = ["mlr", "svm", "dt", "rf", "xgb", "knn", "ann", "gam", "gpr", "gbr", "adaboost"]
    
    model_choices_display = model_choices.copy()
    if LGBM_INSTALLED: 
        model_choices.append("lgbm")
        model_choices_display.append("lgbm")
    if CATBOOST_INSTALLED: 
        model_choices.append("catb")
        model_choices_display.append("catb")

    # --- Mostrar menú si no hay argumentos ---
    if len(sys.argv) == 1:
        display_options_menu(scaler_choices, reduction_choices, model_choices_display)
        sys.exit(0)
    ...
    parser.add_argument("--scaler", required=True, choices=scaler_choices, help="Tipo de escalado")
    parser.add_argument("--reduction", required=True, choices=reduction_choices, help="Tipo de reducción/selección de dimensionalidad")
    ...
    # --- INICIO DE GRÁFICOS DE REDUCCIÓN ---
    if args.reduction in ['pca', 'pca_2'] and reducer_obj is not None:
        pca_loadings_path = os.path.join(output_dir_name, "pca_loadings.png")
        plot_pca_loadings(reducer_obj, original_feature_names, pca_loadings_path)
        print(f"Gráfico de loadings de PCA guardado en {pca_loadings_path}")
    elif args.reduction == 'plsr' and reducer_obj is not None:
        plsr_loadings_path = os.path.join(output_dir_name, "plsr_loadings.png")
        plot_plsr_loadings(reducer_obj, original_feature_names, plsr_loadings_path)
        print(f"Gráfico de loadings de PLSR guardado en {plsr_loadings_path}")
    elif args.reduction in ['sparsepca', 'sparsepca_2'] and reducer_obj is not None:
        sparsepca_loadings_path = os.path.join(output_dir_name, "sparsepca_loadings.png")
        plot_sparsepca_loadings(reducer_obj, original_feature_names, sparsepca_loadings_path)
        print(f"Gráfico de loadings de SparsePCA guardado en {sparsepca_loadings_path}")
    # --- FIN DE GRÁFICOS DE REDUCCIÓN ---
    ...
    with open(stats_path, "w", encoding="utf-8") as f:
        ...
        # Guardar loadings de PCA en el archivo de estadísticas
        if args.reduction in ['pca', 'pca_2'] and reducer_obj is not None:
            f.write("\nComposición de los Componentes Principales (Loadings):\n")
            loadings_pca = reducer_obj.components_
            n_components_pca = reducer_obj.n_components_
            component_labels_pca = [f"PC{i+1}" for i in range(n_components_pca)]
            loadings_df_pca = pd.DataFrame(loadings_pca.T, columns=component_labels_pca, index=original_feature_names)
            f.write(loadings_df_pca.to_string(float_format="%.4f"))
            f.write("\n\n")

        # Guardar loadings de PLSR en el archivo de estadísticas
        ...
        # Guardar loadings de SparsePCA en el archivo de estadísticas
        if args.reduction in ['sparsepca', 'sparsepca_2'] and 'sparsepca_loadings' in reduction_info:
            f.write("\nComposición de los Componentes Dispersos (Loadings):\n")
            loadings_dict = reduction_info['sparsepca_loadings']
            loadings_df = pd.DataFrame(loadings_dict)
            f.write(loadings_df.to_string(float_format="%.4f"))
            f.write("\n\n")

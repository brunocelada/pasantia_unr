...
then
    echo "micro Intel"
    g{gType}root=/opt/ohpc/pub/apps/software/Gaussian/{gType}/{microIntel}-enabled
else
    echo "micro AMD"
    g{gType}root=/opt/ohpc/pub/apps/software/Gaussian/{gType}/{microAMD}-enabled
fi
...
export g{gType}root GAUSS_SCRDIR
. $g{gType}root/g{gType}/bsd/g{gType}.profile
...
def crear_archivo_sh(file, part, gType, microIntel, microAMD, cpuName, nprocshared, tiempo, fold, send_notif):
...
"""
    ...
    with open(f"{job_name}.sh", "w", encoding="utf-8") as f:
        f.write(sh_template.format(job_name=job_name, part=part, gType=gType, microIntel=microIntel, microAMD=microAMD, nprocshared=nprocshared, 
                                   cpuName=cpuName, tiempo=tiempo, commands=commands, calc_notif=calc_notif, final_notif=final_notif))
...
def main():
    ...
    # Comprobar tipo de Gaussian y modificar detalles en base al número ingresado.
    gaussianType = int(input("¿Qué Gaussian se usará? 1: G09, 2:G16  "))
    while gaussianType not in [1, 2]:
        print("Tipeaste mal")
        gaussianType = int(input("¿Qué Gaussian se usará? 1: G09, 2:G16  "))
    ...
    folder = input("Folder: ")
    fold = f"g{gType} /home/{user}/{folder}/" if folder else f"g{gType} /home/{user}/"

    # Aclaración
    print("\nRecordar que se trabaja con 1 input por sh en este Script.\n")
    ...
    for file in files:
        crear_archivo_sh(file, part, gType, microIntel, microAMD, cpuName, nprocshared, tiempo, fold, send_notif)

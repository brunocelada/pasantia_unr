...
def z_score(data):
    ...
    if std_val == 0:
        logging.info("z_score: desviación estándar 0 detectada, se normaliza con ceros.")
...
# --- Títulos ---
# La primera columna: identificador; de la 2 a la 9: features para la ecuación; la 10: ΔΔG‡ medido (valor original)
titles = ["entry (global)",
          "Polarizability",
          "NBO (X)",
          "B5 (avg)",
...
# --- Función para leer el archivo y crear un dict ---
        ...
        with open(archivo_txt, 'r', encoding='utf-8') as f:
            ...
            for linea in f:
                ...
                try:
                    ...
                    logging.warning(f"Error al convertir a float en la línea: {linea.strip()}")
                    continue
                # Primeras 8 columnas: features; la 9ª columna: ΔΔG‡ medido
...
# --- Función para aplicar una cadena de normalización a una lista de datos ---
def aplicar_cadena_normalizacion(data, chain):
    result = data[:]
    if chain:
        for method in chain:
            result = normalizations_available[method](result)
    return result

# --- Generar todas las cadenas posibles para una columna ---
def generate_possible_chains(max_length=3):
    keys = list(normalizations_available.keys())
    chains = [()]  # Identidad
    for r in range(1, max_length+1):
        for comb in itertools.product(keys, repeat=r):
            chains.append(comb)
    return chains

# --- Generador de combinaciones para las 8 columnas ---
def generate_full_combinations_generator(max_length=3):
    possible_chains = generate_possible_chains(max_length)
    columnas = titles[1:9]
    for combo in itertools.product(possible_chains, repeat=8):
        yield dict(zip(columnas, combo))

# --- Procesar combinaciones en batches ---
def process_batches(data_dict, max_length=3, batch_size=100):
    gen = generate_full_combinations_generator(max_length)
    batch_number = 0
    while True:
        batch = list(itertools.islice(gen, batch_size))
        if not batch:
            break
        resultados, keys_ordenados = calcular_resultados_combinaciones_general(data_dict, batch)
...
# --- Calcular los resultados de la ecuación para cada combinación ---
def calcular_resultados_combinaciones_general(data_dict, combinaciones):
    keys_ordenados = sorted(data_dict.keys(), key=lambda x: float(x))
    feature_cols = titles[1:9]
    # Extraer los valores originales para cada feature, en orden
    columnas_originales = {col: [data_dict[k][col] for k in keys_ordenados] for col in feature_cols}
    
    resultados = {}
    for combo in combinaciones:
        # Descripción de la combinación: para cada columna, mostrar la cadena o "identidad"
        combo_descr = "; ".join(f"{col}: {'-'.join(chain) if chain else 'identidad'}" 
                                 for col, chain in combo.items())
        columnas_norm = {}
        for col in feature_cols:
            chain = combo[col]
            columnas_norm[col] = aplicar_cadena_normalizacion(columnas_originales[col], chain)
        calculados = []
...
# --- Función principal ---
def main():
    logging.info("\n\n-------NUEVO CÁLCULO DE NORMALIZACIÓN Y ECUACIÓN (por batches)-------\n")
    ...
    if not data_dict:
        ...
        return
    # Procesar todas las combinaciones posibles en batches (por ejemplo, 100 combinaciones por lote)
    process_batches(data_dict, max_length=3, batch_size=100)
    print("\nProceso finalizado.\n")
